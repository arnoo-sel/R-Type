...
Ship::Ship(int id, sf::Texture& img, Ship::Color color)
{
...
{
    _limit = SPEED / (1 + lclock.getElapsedTime().asSeconds());
    lclock.restart();
}

void Ship::moveToCoord()
{
    float t = fclock.getElapsedTime().asSeconds();
